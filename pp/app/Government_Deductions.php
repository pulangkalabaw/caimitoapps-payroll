<?php
function sss_deduction()
{
    $government_deductions[0]['from'] = 0.00;
    $government_deductions[0]['to'] = 6479.99;
    $government_deductions[0]['ee'] = 80.00;
    $government_deductions[0]['er'] = 160.00;

    $government_deductions[1]['from'] = 2250.00;
    $government_deductions[1]['to'] = 2749.99;
    $government_deductions[1]['ee'] = 100.00;
    $government_deductions[1]['er'] = 200.00;

    $government_deductions[2]['from'] = 2750.00;
    $government_deductions[2]['to'] = 3249.99;
    $government_deductions[2]['ee'] = 120.00;
    $government_deductions[2]['er'] = 240.00;

    $government_deductions[3]['from'] = 3250.00;
    $government_deductions[3]['to'] = 3749.99;
    $government_deductions[3]['ee'] = 140.00;
    $government_deductions[3]['er'] = 280.00;

    $government_deductions[4]['from'] = 3750.00;
    $government_deductions[4]['to'] = 4249.99;
    $government_deductions[4]['ee'] = 160.00;
    $government_deductions[4]['er'] = 320.00;

    $government_deductions[5]['from'] = 4250.00;
    $government_deductions[5]['to'] = 4749.99;
    $government_deductions[5]['ee'] = 180.00;
    $government_deductions[5]['er'] = 360.00;

    $government_deductions[6]['from'] = 4750.00;
    $government_deductions[6]['to'] = 5249.99;
    $government_deductions[6]['ee'] = 200.00;
    $government_deductions[6]['er'] = 400.00;

    $government_deductions[7]['from'] = 5250.00;
    $government_deductions[7]['to'] = 5749.99;
    $government_deductions[7]['ee'] = 220.00;
    $government_deductions[7]['er'] = 440.00;

    $government_deductions[8]['from'] = 5750.00;
    $government_deductions[8]['to'] = 6249.99;
    $government_deductions[8]['ee'] = 240.00;
    $government_deductions[8]['er'] = 480.00;

    $government_deductions[9]['from'] = 6250.00;
    $government_deductions[9]['to'] = 6479.99;
    $government_deductions[9]['ee'] = 260.00;
    $government_deductions[9]['er'] = 520.00;

    $government_deductions[10]['from'] = 6750.00;
    $government_deductions[10]['to'] = 7249.99;
    $government_deductions[10]['ee'] = 280.00;
    $government_deductions[10]['er'] = 560.00;

    $government_deductions[11]['from'] = 7250.00;
    $government_deductions[11]['to'] = 7749.99;
    $government_deductions[11]['ee'] = 300.00;
    $government_deductions[11]['er'] = 600.00;

    $government_deductions[12]['from'] = 7750.00;
    $government_deductions[12]['to'] = 8249.99;
    $government_deductions[12]['ee'] = 320.00;
    $government_deductions[12]['er'] = 640.00;

    $government_deductions[13]['from'] = 8250.00;
    $government_deductions[13]['to'] = 8749.99;
    $government_deductions[13]['ee'] = 340.00;
    $government_deductions[13]['er'] = 680.00;

    $government_deductions[14]['from'] = 8750.00;
    $government_deductions[14]['to'] = 9249.99;
    $government_deductions[14]['ee'] = 360.00;
    $government_deductions[14]['er'] = 720.00;

    $government_deductions[15]['from'] = 9250.00;
    $government_deductions[15]['to'] = 9749.99;
    $government_deductions[15]['ee'] = 380.00;
    $government_deductions[15]['er'] = 760.00;

    $government_deductions[16]['from'] = 9750.00;
    $government_deductions[16]['to'] = 10249.99;
    $government_deductions[16]['ee'] = 400.00;
    $government_deductions[16]['er'] = 800.00;

    $government_deductions[17]['from'] = 10250.00;
    $government_deductions[17]['to'] = 10749.99;
    $government_deductions[17]['ee'] = 420.00;
    $government_deductions[17]['er'] = 840.00;

    $government_deductions[18]['from'] = 10750.00;
    $government_deductions[18]['to'] = 11249.99;
    $government_deductions[18]['ee'] = 440.00;
    $government_deductions[18]['er'] = 880.00;

    $government_deductions[19]['from'] = 11250.00;
    $government_deductions[19]['to'] = 11749.99;
    $government_deductions[19]['ee'] = 460.00;
    $government_deductions[19]['er'] = 920.00;

    $government_deductions[20]['from'] = 11750.00;
    $government_deductions[20]['to'] = 12249.99;
    $government_deductions[20]['ee'] = 480.00;
    $government_deductions[20]['er'] = 960.00;

    $government_deductions[21]['from'] = 12250.00;
    $government_deductions[21]['to'] = 12749.99;
    $government_deductions[21]['ee'] = 500.00;
    $government_deductions[21]['er'] = 1000.00;

    $government_deductions[22]['from'] = 12750.00;
    $government_deductions[22]['to'] = 13249.99;
    $government_deductions[22]['ee'] = 520.00;
    $government_deductions[22]['er'] = 1040.00;

    $government_deductions[23]['from'] = 13250.00;
    $government_deductions[23]['to'] = 13749.99;
    $government_deductions[23]['ee'] = 540.00;
    $government_deductions[23]['er'] = 1080.00;

    $government_deductions[24]['from'] = 13750.00;
    $government_deductions[24]['to'] = 14249.99;
    $government_deductions[24]['ee'] = 560.00;
    $government_deductions[24]['er'] = 1120.00;

    $government_deductions[25]['from'] = 14250.00;
    $government_deductions[25]['to'] = 14749.99;
    $government_deductions[25]['ee'] = 580.00;
    $government_deductions[25]['er'] = 1160.00;

    $government_deductions[26]['from'] = 14750.00;
    $government_deductions[26]['to'] = 15249.99;
    $government_deductions[26]['ee'] = 600.00;
    $government_deductions[26]['er'] = 1200.00;

    $government_deductions[27]['from'] = 15250.00;
    $government_deductions[27]['to'] = 15749.99;
    $government_deductions[27]['ee'] = 620.00;
    $government_deductions[27]['er'] = 1240.00;

    $government_deductions[28]['from'] = 15750.00;
    $government_deductions[28]['to'] = 16249.99;
    $government_deductions[28]['ee'] = 640.00;
    $government_deductions[28]['er'] = 1280.00;

    $government_deductions[29]['from'] = 16250.00;
    $government_deductions[29]['to'] = 16749.99;
    $government_deductions[29]['ee'] = 660.00;
    $government_deductions[29]['er'] = 1320.00;

    $government_deductions[30]['from'] = 16750.00;
    $government_deductions[30]['to'] = 17249.99;
    $government_deductions[30]['ee'] = 680.00;
    $government_deductions[30]['er'] = 1360.00;

    $government_deductions[31]['from'] = 17250.00;
    $government_deductions[31]['to'] = 17749.99;
    $government_deductions[31]['ee'] = 700.00;
    $government_deductions[31]['er'] = 1400.00;

    $government_deductions[32]['from'] = 17750.00;
    $government_deductions[32]['to'] = 18249.99;
    $government_deductions[32]['ee'] = 720.00;
    $government_deductions[32]['er'] = 1440.00;

    $government_deductions[33]['from'] = 18250.00;
    $government_deductions[33]['to'] = 18749.99;
    $government_deductions[33]['ee'] = 740.00;
    $government_deductions[33]['er'] = 1480.00;

    $government_deductions[34]['from'] = 18750.00;
    $government_deductions[34]['to'] = 19249.99;
    $government_deductions[34]['ee'] = 760.00;
    $government_deductions[34]['er'] = 1520.00;

    $government_deductions[35]['from'] = 19250.00;
    $government_deductions[35]['to'] = 19749.99;
    $government_deductions[35]['ee'] = 780.00;
    $government_deductions[35]['er'] = 1560.00;

    $government_deductions[36]['from'] = 19750.00;
    $government_deductions[36]['to'] = 'and above';
    $government_deductions[36]['ee'] = 800.00;
    $government_deductions[36]['er'] = 1600.00;

    return $government_deductions;
}

function withholding_tax(){

    // WITHHOLDING TAX FOR DAILY
    $witholding_tax[0]['wage_type'] = 'daily';
    $witholding_tax[0]['compensation_range_from'] = 0;
    $witholding_tax[0]['compensation_range_to'] = 684;
    $witholding_tax[0]['prescribed_withholding_tax'] = json_encode([0, 0]);

    $witholding_tax[1]['wage_type'] = 'daily';
    $witholding_tax[1]['compensation_range_from'] = 685;
    $witholding_tax[1]['compensation_range_to'] = 1095;
    $witholding_tax[1]['prescribed_withholding_tax'] = json_encode([.20, 0]);

    $witholding_tax[2]['wage_type'] = 'daily';
    $witholding_tax[2]['compensation_range_from'] = 1096;
    $witholding_tax[2]['compensation_range_to'] = 2191;
    $witholding_tax[2]['prescribed_withholding_tax'] = json_encode([.25, 82.19]);

    $witholding_tax[3]['wage_type'] = 'daily';
    $witholding_tax[3]['compensation_range_from'] = 2192;
    $witholding_tax[3]['compensation_range_to'] = 5478;
    $witholding_tax[3]['prescribed_withholding_tax'] = json_encode([.30, 356.16]);

    $witholding_tax[4]['wage_type'] = 'daily';
    $witholding_tax[4]['compensation_range_from'] = 5479;
    $witholding_tax[4]['compensation_range_to'] = 21917;
    $witholding_tax[4]['prescribed_withholding_tax'] = json_encode([.32, 1342.47]);

    $witholding_tax[5]['wage_type'] = 'daily';
    $witholding_tax[5]['compensation_range_from'] = 21918;
    $witholding_tax[5]['compensation_range_to'] = 'and above';
    $witholding_tax[5]['prescribed_withholding_tax'] = json_encode([.35, 6602.74]);

    // WITHHOLDING TAX FOR SEMI-MONTHLY
    $witholding_tax[6]['wage_type'] = 'semi-monthly';
    $witholding_tax[6]['compensation_range_from'] = 0;
    $witholding_tax[6]['compensation_range_to'] = 10416;
    $witholding_tax[6]['prescribed_withholding_tax'] = json_encode([0, 0]);

    $witholding_tax[7]['wage_type'] = 'semi-monthly';
    $witholding_tax[7]['compensation_range_from'] = 10417;
    $witholding_tax[7]['compensation_range_to'] = 16666;
    $witholding_tax[7]['prescribed_withholding_tax'] = json_encode([.20, 0]);

    $witholding_tax[7]['wage_type'] = 'semi-monthly';
    $witholding_tax[7]['compensation_range_from'] = 16667;
    $witholding_tax[7]['compensation_range_to'] = 33332;
    $witholding_tax[7]['prescribed_withholding_tax'] = json_encode([.25, 1250.00]);

    $witholding_tax[8]['wage_type'] = 'semi-monthly';
    $witholding_tax[8]['compensation_range_from'] = 33333;
    $witholding_tax[8]['compensation_range_to'] = 83332;
    $witholding_tax[8]['prescribed_withholding_tax'] = json_encode([.30, 5416.67]);

    $witholding_tax[9]['wage_type'] = 'semi-monthly';
    $witholding_tax[9]['compensation_range_from'] = 83333;
    $witholding_tax[9]['compensation_range_to'] = 333332;
    $witholding_tax[9]['prescribed_withholding_tax'] = json_encode([.32, 20416.67]);

    $witholding_tax[10]['wage_type'] = 'semi-monthly';
    $witholding_tax[10]['compensation_range_from'] = 333333;
    $witholding_tax[10]['compensation_range_to'] = 'and above';
    $witholding_tax[10]['prescribed_withholding_tax'] = json_encode([.35, 100416.67]);

}
